...
export class UserFormComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['user'] && this.userForm) this.patchForm();
  }

  private buildForm(): void {
    const arrValidator = (c: any) =>
      !c.value || !Array.isArray(c.value) || !c.value.length
        ? { required: true }
        : null;
    this.userForm = this.fb.group(
      {
        name: [
          ...
          [
            ...
            (c: any) =>
              c.value && /\d/.test(c.value) ? { hasNumbers: true } : null,
          ],
        ],
        email: [
          '',
        ...
        creditCard: [
          '',
          [Validators.required, Validators.pattern(/^(\d{16}|\d{4})$/)],
        ],
      ...
      },
      {
        validators: (g: FormGroup) => {
          const p = g.get('password')?.value,
            c = g.get('confirmPassword')?.value;
          return !p && !c ? null : p === c ? null : { mismatch: true };
        },
      },
    );
    ...
    this.userForm
      .get('state')
      ?.valueChanges.subscribe((s) => this.onStateChange(s));
    this.patchForm();
  }
  ...
  private patchForm(): void {
    if (this.user) {
      ...
      const dob =
        this.user.dob && typeof this.user.dob === 'string'
          ? this.user.dob.split('T')[0]
          : '';
  ...
  onStateChange(state: string): void {
    ...
    this.cities = (this.citiesByState[state] || []).map((c) => ({
      label: c,
      value: c,
    }));
    const curr = this.userForm.get('city')?.value;
    if (curr && !this.cities.find((c) => c.value === curr))
    ...
    req.subscribe({
      ...
      error: (e) => {
        this.toastService.show(
          'error',
          'Save Failed',
          e?.error?.message || 'Unable to save user. Please try again.',
          5000,
        );
        this.submitting = false;
      },
